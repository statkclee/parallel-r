...
``` {r fp-dedup-01, warning=FALSE, tidy=FALSE}
...
``` {r fp-dedup-02, warning=FALSE, tidy=FALSE}
...
``` {r fp-functions-argument-ex01, warning=FALSE, tidy=FALSE}
col_median <- 
	function(df) {
...
``` {r fp-functions-argument-ex02, warning=FALSE, tidy=FALSE}
col_median <- 
	function(df) {
		output <- numeric(length(df))
		for (i in seq_along(df)) {
...
``` {r fp-functions-argument-ex03, warning=FALSE, tidy=FALSE}
col_median <- 
	function(df, fun) {
		output <- numeric(length(df))
		for (i in seq_along(df)) {
...
``` {r fp-functions-argument-ex04, warning=FALSE, tidy=FALSE}
col_summary <- 
	function(df, fun) {
		output <- numeric(length(df))
		for (i in seq_along(df)) {
